...
# Red Virtual
resource "azurerm_virtual_network" "red_cloud" {
  name                = "virtualnetwork"
...
resource "azurerm_private_dns_zone" "dns_zone" {
  ...
  name                = "luiguivm.mysql.database.azure.com"
  resource_group_name = azurerm_resource_group.test_cloudazure.name
}
...
resource "azurerm_public_ip" "test_cloudazure" {
  ...
  sku                 = "Standard"
  domain_name_label   = "luiguivm${count.index}"
}
...
variable "private_key_path" {
  type    = string
  default = "C:\\terraform\\keys\\private" /*Cambiar por clave generada por putty*/
}

# Definir y configurar una máquina virtual 
resource "azurerm_virtual_machine" "test_cloudazure" {
  count                 = 2
  name                  = "ServicesCMS${count.index}"
  location              = azurerm_resource_group.test_cloudazure.location
  ...
  os_profile {
    computer_name  = "Luigui"
    admin_username = "luigui"
    admin_password = "Luigui2001."
  }
...
resource "azurerm_log_analytics_workspace" "logs_cloudazure" {
  ...
  name                = "log-analytics-workspace-${count.index}"
  location            = azurerm_resource_group.test_cloudazure.location
  resource_group_name = azurerm_resource_group.test_cloudazure.name
